...
public class IntegerArray extends Array {
    ...
    public Array reversed() {
        Array newArray = new IntegerArray(array.length);
    ...
    public Array sorted(boolean asc) {
        ...
        while (moved) {
            ...
            for (int i = 0; i < sortedArray.size() - 1; i++) {
                boolean condition;
                if (asc)
                    condition = (int) sortedArray.array[i] > (int) sortedArray.array[i + 1];
                else
                    condition = (int) sortedArray.array[i] < (int) sortedArray.array[i + 1];

                if (condition) {
                    sortedArray.array[i] = (int) sortedArray.array[i] + (int) sortedArray.array[i + 1];
                    sortedArray.array[i + 1] = (int) sortedArray.array[i] - (int) sortedArray.array[i + 1];
                    sortedArray.array[i] = (int) sortedArray.array[i] - (int) sortedArray.array[i + 1];
                    moved = true;
                }
            }
    ...
    public Array copyOf(Array array) {
        Array copy = new IntegerArray(array.capacity());
    ...
    public String toString() {
        ...
        for (int i = 0; i <= lastInsertedIndex; i++) {
            stringifier.append(String.format("%d,", array[i]));
        }
